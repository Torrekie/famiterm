...
class Ppu:
    ...
    background_mapper_state: tuple[str, tuple[int, int, int, int]] | None = None
    ...
    def update_tiles(self) -> None:
        ...
        mapper_state = (self.cartridge.mirroring, self.cartridge.chr_cache_key)
        if self.background_mapper_state != mapper_state:
            self.background_mapper_state = mapper_state
            self.background_pattern_table_address_changed = True
        # Draw everything
        if (
...
class Cpu:
    ...
    def cpu_read(self, addr: int) -> int:
        ...
        if addr == 0x4017:
            return 0
        # Disabled APU/IO and cartridge expansion area
        if 0x4018 <= addr < 0x6000:
            return 0
        # Invalid access
        if 0x6000 <= addr < 0x8000:
            return self.cartridge.cpu_read(addr)
